...
The following must be resolved before breaking this vision into epics and stories:

1. **Stakeholder alignment** — OHM Agenda is a shared tool. All stakeholders must explicitly agree to migrate before V1 development begins. **Open.**

2. **Data migration plan** — The Google Sheet contains fields absent from OHM Agenda (phone, address, job, birth date and place, nationality, first inscription date). Some may conflict with existing database records. A reconciliation plan will be defined during V1 implementation. The migration must import the complete fee payment history (covering all historical seasons) so that first inscription dates derived from the earliest recorded fee payment are accurate; first inscription date remains derived-only post-migration.

3. **GDPR requirements** — Documented in [`specs/goals/gdpr.md`](gdpr.md). Reviewed.

## Open Questions

1. **OVH deployment target** — OVH offers several products (VPS, shared hosting, managed
   PaaS). The deployment target must be decided before technical specs can be written.
...
- Statistics page (data will be captured correctly; UI deferred)
- Community manager role (decision deferred)
...
- [ ] Write technical specs in `specs/technical-specs/`
...
- [ ] Iterate until a fresh-context prompt passes the gate:
  > "Review `specs/functional-specs/` and `specs/technical-specs/` as if you were about to decompose them into implementation tasks. Flag anything ambiguous, missing, or that would force you to guess."
...
### Functional
- Secure
- GDPR compliant (see Prerequisites)

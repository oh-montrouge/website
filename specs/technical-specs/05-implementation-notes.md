...
--    If consent is false, clear any pre-populated phone/address (e.g. migrated data).
UPDATE accounts
SET status = 'active',
    password_hash = $1,          -- Argon2id hash
    phone_address_consent = $2,  -- from checkbox
    phone   = CASE WHEN $2 THEN phone   ELSE NULL END,
    address = CASE WHEN $2 THEN address ELSE NULL END
WHERE id = $3;
...
`account_id` column write — see Sessions above), then redirect to `/evenements`.

...
| Password | Post-invite | Set by the musician during the invite flow. Never retrievable in plain text. Cleared on anonymization. Minimum of 22 characters mixing lower and upper cases, numerical and special characters. |
...
| Season | Yes | — |
| Amount | Yes | Currency: EUR. Two decimal places. Must be ≥ 0. Zero is permitted (e.g., honorary membership or subsidised fee). |
...
| RSVP | Yes | — |
| EventField | Yes | — |
| Value | Yes | Stored as text regardless of field type. For `choice` fields, the stored value is the `EventFieldChoice` ID (cast to text). |

**Constraint:** At most one response per (RSVP, EventField) pair (uniqueness guard, not mandatory
presence). Optional fields may have no response record for a given RSVP. Responses are deleted
when the RSVP state changes from `yes` to `no` or `maybe`.
...
Cross-entity invariants that must hold at all times. Each spans more than one entity and
cannot be enforced by a single entity in isolation. Any new code path that writes to the
affected entities must verify that its operation preserves the relevant invariant.

| # | Invariant | Entities spanned | Gates |
|---|-----------|-----------------|-------|
| I1 | At most one FeePayment per account per season | Account, FeePayment, Season | Recording a fee payment: rejected if a payment already exists for the (account, season) pair. Prompt to edit existing payment instead. |
| I2 | Exactly one RSVP per (account, event) pair | Account, Event, RSVP | RSVP records are system-created, not directly user-created. The system guarantees uniqueness at creation time (event creation, account activation). |
| I3 | At least one `active` account holds the admin role at all times | Account, account roles | Role revocation, anonymization of an admin account, deletion of a pending admin account: each blocked if it would leave zero active admins. |
| I4 | Exactly one Season is designated current at all times | Season | The current designation cannot be removed without simultaneously transferring it to another season. |

**Enforcement references:**
- I1 — `04-fee-payments.md` § Recording a Fee Payment
- I2 — `05-events-and-rsvp.md` § RSVP Record Eligibility
- I3 — `01-account-lifecycle.md` § Last-Admin Protection; `02-musician-management.md`
- I4 — `03-season-management.md` § Designating the Current Season
